...
const TEST_CASES: {
...
}[] = [
  {
    ...
    tests: [
      ...
      {
        description: "NaN and NaN are NOT equal",
        value1: NaN,
        value2: NaN,
        equal: false,
      },
  ...
  },
  {
    ...
    tests: [
      ...
      {
        description: "different functions are equal",
        value1: func1,
        value2: func2,
        equal: true,
      },
...
describe("deepEqual", () => {
  TEST_CASES.forEach(({ description: testSetDescription, tests }) => {
    describe(testSetDescription, () => {
      tests.forEach(
        ({ description, value1, value2, equal: expectedEquality }) => {
          test(description, () => {
            const isEqual = deepEqual(value1, value2);

            if (isEqual !== expectedEquality) {
              throw new Error(
                `Equality is NOT expected, isEqual=[${isEqual}] expectedEquality=[${expectedEquality}]`,
              );
            }
          });
        },
